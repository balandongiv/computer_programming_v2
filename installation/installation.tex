...
\begin{enumerate}
    ...
    \item Once it has opened, you should see an editor interface in the browser together with a terminal area.
\end{enumerate}
\begin{figure}
    \centering
...
\begin{enumerate}
    \item Open the terminal in Codespaces.

    \item Locate the file named \texttt{first\_run.py}.

    \item Run the script.

    You may use:
    \begin{quote}
        \texttt{python first\_run.py}
    \end{quote}
    or, if needed,
    \begin{quote}
        \texttt{python3 first\_run.py}
    \end{quote}

    \item Wait for the program to execute.

    \item If everything is okay, you should see the output in the terminal.

    \item If the output appears normally, this means your Codespace is working properly and you are ready to begin.
\end{enumerate}

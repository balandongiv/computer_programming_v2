...
Session 4 taught students how to move code into functions. That was the first major refactor: instead of keeping everything in one long script, students learned to divide the work into smaller parts such as prediction, counter updates, and summary printing.

Session 5 continues that same refactoring mindset. This time, the classifier rule itself does \textbf{not} change. What changes is the \textbf{interface} of the program. Instead of forcing one fixed run style, we want the same pipeline to support:
\begin{itemize}
    \item a default run,
    \item a run with positional overrides,
    \item a run with keyword overrides,
    \item and a run where the threshold is learned from the dataset through a helper function.
\end{itemize}
